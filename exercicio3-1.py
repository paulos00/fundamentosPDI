...
def main():
    ...
    axes[1].set_title(f"Transl. ({dx}, {dy}) - com Pillow")

    axes[2].imshow(translate_opencv(npImg, dx, dy), cmap='gray')
    axes[2].set_title(f"Transl. ({dx}, {dy}) - com OpenCV")

    axes[3].imshow(translate_numpy(npImg, dx, dy), cmap='gray')
    axes[3].set_title(f"Transl. ({dx}, {dy}) - com Numpy")

    axes[4].imshow(translate_scipy(npImg, dx, dy), cmap='gray')
    axes[4].set_title(f"Transl. ({dx}, {dy}) - com Scipy")
